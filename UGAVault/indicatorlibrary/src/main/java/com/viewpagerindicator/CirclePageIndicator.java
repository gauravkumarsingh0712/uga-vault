...
public class CirclePageIndicator extends View implements PageIndicator {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			...
			// Draw text
				int pageNumber=iLoop;
				pageNumber=pageNumber+1;
//$				canvas.drawText(""+pageNumber, dX, dY + (pageFillRadius / 2), mPaintText);
			canvas.drawText("", dX, dY + (pageFillRadius / 2), mPaintText);
		...
		int pageFillCount=mCurrentPage+1;
//$		canvas.drawText(""+pageFillCount, dX, dY + (pageFillRadius / 2), mPaintText);
		canvas.drawText("", dX, dY + (pageFillRadius / 2), mPaintText);
